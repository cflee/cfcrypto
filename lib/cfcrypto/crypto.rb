...
module Cfcrypto
  ...
  def self.aes_encrypt_oracle(msg)
    rand = Random.new
    # add the random amount of prefix and suffix data
    padded_msg = rand.bytes(5 + rand.rand(5)) + msg + rand.bytes(5 + rand.rand(5))
    # random coin to decide ECB or CBC
    if rand.rand(2).zero?
      [aes_ecb_encrypt(generate_key(16), padded_msg), 0]
    else
      [aes_cbc_encrypt(generate_key(16), generate_key(16), padded_msg), 1]
    end
  end
